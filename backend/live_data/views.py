...
def weather(request):
    ...
    try:
        ...
        return JsonResponse({'error': 'RequestError', 'message': str(req_err)}, status=400)
    except ValueError as json_err:
        return JsonResponse({'error': 'JSONError', 'message': str(json_err)}, status=500)
...
def currency(request):
    ...
    if not from_currency:
        return JsonResponse({'error': 'ValidationError', 'message': "Missing 'from' parameter"}, status=400)
    ...
    try:
        ...
        if 'result' not in data or data['result'] is None:
            return JsonResponse({'error': 'APIError', 'message': 'Currency conversion failed'}, status=400)
